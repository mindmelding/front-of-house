...
def read_state(o):
    p = state_path(o)
    st = {"setup": "incomplete", "answered": [], "last_distill": None, "in_motion_refreshed": None, "adapter": None, "host": None, "preshift": "offer"}
    if not p.exists():
        return st
    m = re.search(r"```json\n(.*?)\n```", p.read_text(), re.S)
    if m:
...
def cmd_status(a):
    ...
    if a.brief:
        if st["setup"] != "complete":
            print("Front of House: overlay setup incomplete. Run the First Shift (FIRST-SHIFT.md).")
        elif warranted:
    ...
    print(f"setup:              {st['setup']}  (answered {len(st.get('answered', []))}/17 interview questions)")
...
def cmd_answer(a):
    ...
    if len(st["answered"]) >= 17:
        st["setup"] = "complete"
    write_state(o, st)
    print(f"saved {a.key} ({len(st['answered'])}/17)")
    return 0
