...
- Fixed: the house (`overlay/house/`, `queue/`, `lexicon.md`, `method.md`) had been committed to the public canon. The allowlist `.gitignore` that `sync init` writes into the overlay re-included them past the canon's `overlay/*` rule. The canon now ignores `overlay/` as a directory, `sync init` no longer overwrites an existing overlay README, and CI fails if anything under `overlay/` other than the README and templates is tracked.
...
- Sync: `engine.py sync` makes the overlay a git repo of learning files only (allowlist), and every decision commits and pushes itself behind the alias audit. The house compounds on GitHub in real time without a customer in it.
